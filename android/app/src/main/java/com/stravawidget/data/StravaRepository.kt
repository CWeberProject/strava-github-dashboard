...
class StravaRepository(context: Context) {
    ...
    companion object {
        private const val DEFAULT_WEEKS = 13

        /**
         * Generate grid dates for the widget.
         * @param weeks Number of weeks (columns) to display
         * @return List of dates organized by day of week, then by week
         */
        fun getGridDates(weeks: Int = DEFAULT_WEEKS): List<LocalDate?> {
            val today = LocalDate.now()
            ...
            val startDate = endDate.minusDays(daysToShow.toLong() - 1)
